...
// ---------------------------------------------------------------------------
// このスクリプトはプロジェクト非依存である。
// プロジェクト固有の事情は全て args 経由で渡す (Workflow スクリプトはファイルシステムへアクセスできないため、設定ファイルを自力で読めない)。
//
// args:
...
//
// この後オーケストレーター (メインループ) が hygiene-check → フルスイート → 不変条件チェック → 重量テストを回してからコミットする。
// テスト実行をこのスクリプトへ足さないこと: 並列エージェントが同時にスイートを起動すると CPU 競合で打ち切られ、赤の原因が判別できなくなる。
// 
// ---------------------------------------------------------------------------
